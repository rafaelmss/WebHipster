...
public class FilmeResource {
    ...
    public ResponseEntity<Filme> createFilme(@Valid @RequestBody Filme filme) throws URISyntaxException {
        ...
        if (filme.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new filme cannot already have an ID")).body(null);
        }
    ...
    @GetMapping("/filmes")
    @Timed
    public ResponseEntity<List<Filme>> getAllFilmes(@ApiParam Pageable pageable) {
    ...
    public ResponseEntity<Filme> getFilme(@PathVariable Long id) {
        ...
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(filme));
    }
